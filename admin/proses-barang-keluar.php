    ...
    if (isset($_POST['kode_barcode']) && !empty($_POST['kode_barcode'])) {
        ...
        if (mysqli_num_rows($check_result) > 0) {
            ...
            if ($data['status'] == 'terjual') {
                ...
                // Update status menjadi terjual/rusak
                $status_baru = ($jenis_penjualan === 'rusak') ? 'rusak' : 'terjual';
                $update_query = "UPDATE barcode_produk SET status = '$status_baru', updated_at = NOW() WHERE kode_barcode = '$kode_barcode'";


                if (mysqli_query($conn, $update_query)) {
                    // Kurangi stok gudang (hanya jika bukan rusak)
                    $kurangiStok = "UPDATE stok_gudang SET jumlah = jumlah - 1 WHERE nama_barang = '{$data['nama_barang']}'";
                    $stok_dikurangi = true;
                    if ($jenis_penjualan === 'rusak') {
                        // Barang rusak tetap kurangi stok, tapi log berbeda
                    ...
                    if (mysqli_query($conn, $kurangiStok)) {
                        ...
                        $log_query = "INSERT INTO log_aktivitas (username, aksi, waktu) VALUES (
                            '$username', 
                            'Memproses barang keluar untuk kode: $kode_barcode - {$data['nama_barang']} ($jenis_text)', 
                            NOW()
                        )";
                        ...
                        $response = [
                            'status' => 'success',
                            'message' => 'Barang berhasil diproses keluar (' . $jenis_text . ')',
                            'data' => [

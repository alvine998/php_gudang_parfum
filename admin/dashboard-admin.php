...
<div class="container-fluid px-4 mt-3">
  <div class="row align-items-center mb-4">
    ...
    <div class="col-md-4 col-12 text-md-end text-center">
      <span class="badge bg-primary p-3 fs-6"><i class="bi bi-person-circle me-2"></i>Admin</span>
    </div>
  </div>
...
<script>
  ...
  if (window.Chart) {
    Chart.register(CanvasBgPlugin, ChartDataLabels);
  }
  ...
  function loadStatistikProduk(start = '', end = '') {
    ...
    fetch(url).then(r => r.json()).then(res => {
      ...
      document.getElementById('statistik-produk-row').innerHTML = stats.map(s => `
        <div class="col-6 col-md-3 col-lg-2 col-xl-1-7 mb-3">
          <div class="card border-0 shadow-sm h-100 animate__animated animate__fadeInUp ${s.label==='Terjual' ? 'cursor-pointer' : ''}"
            ...
            <div class="card-body text-center p-2 p-md-3">
              <div class="mb-2">
                <span class="rounded-circle bg-${s.color} bg-opacity-10 d-inline-flex align-items-center justify-content-center" style="width:40px;height:40px;">
                  <i class="bi bi-${s.icon} fs-4 text-${s.color}"></i>
                </span>
              </div>
              <div class="fw-bold fs-5 mb-1 text-${s.color}">${s.value}</div>
      ...
      // Tambahkan CSS custom untuk 7 kolom di xl
      if (!document.getElementById('col-xl-1-7-style')) {
        const style = document.createElement('style');
        style.id = 'col-xl-1-7-style';
        style.innerHTML = `
          @media (min-width: 1200px) {
            .col-xl-1-7 { flex: 0 0 14.2857%; max-width: 14.2857%; }
          }
        `;
  ...
  (function() {
    ...
    btnCSV.addEventListener('click', async () => {
      ...
      const a = Object.assign(document.createElement('a'), {
        href: url,
        download: 'terjual_per_varian.csv'
      });

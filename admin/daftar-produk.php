            ...
            <div class="card shadow card-hover">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-3" id="filter-username-col" style="display:none;">
                            <label for="filter-username" class="form-label">Pilih User:</label>
                            <select class="form-select" id="filter-username" disabled>
                                <option value="">Pilih User</option>
                            </select>
                        </div>
                        <div class="col-md-3" id="filter-toko-col" style="display:none;">
                            <label for="filter-toko" class="form-label">Pilih Toko:</label>
                            <select class="form-select" id="filter-toko" disabled>
                                <option value="">Pilih Toko</option>
                            </select>
                        </div>
                        <div class="col-md-3">
                            <label for="filter-nama" class="form-label">Cari Nama Barang / Kode Qr:</label>
                            <input type="text" class="form-control" id="filter-nama" placeholder="Masukkan nama barang / kode QR...">
                        </div>
                        <div class="col-md-3">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.getElementById('filter-status').addEventListener('change', function() {
            ...
            const userCol = document.getElementById('filter-username-col');
            const userSelect = document.getElementById('filter-username');
            const tokoCol = document.getElementById('filter-toko-col');
            const tokoSelect = document.getElementById('filter-toko');

            // Reset filter
            userCol.style.display = 'none';
            userSelect.value = '';
            tokoCol.style.display = 'none';
            tokoSelect.value = '';

            if (val === 'di_spg' || val === 'di_collecting') {
                userCol.style.display = '';
                userSelect.innerHTML = '<option value="">Memuat...</option>';
                userSelect.disabled = true;
                // Ambil user by role
                fetch('get-user-by-role.php?role=' + (val === 'di_spg' ? 'spg' : 'collecting'))
                    .then(r => r.json())
                    .then(res => {
                        if (res.status === 'success') {
                            userSelect.innerHTML = '<option value="">Semua User</option>' +
                                res.data.map(u => `<option value="${u.username}">${u.username}</option>`).join('');
                            userSelect.disabled = false;
                        } else {
                            userSelect.innerHTML = '<option value="">Tidak ada user</option>';
                            userSelect.disabled = true;
                        }
                    });
            } else if (val === 'di_toko') {
                tokoCol.style.display = '';
                tokoSelect.innerHTML = '<option value="">Memuat...</option>';
                tokoSelect.disabled = true;
                // Ambil daftar toko
                fetch('get-toko-list.php')
                    .then(r => r.json())
                    .then(res => {
                        if (res.status === 'success') {
                            tokoSelect.innerHTML = '<option value="">Semua Toko</option>' +
                                res.data.map(t => `<option value="${t.nama_toko}">${t.nama_toko}</option>`).join('');
                            tokoSelect.disabled = false;
                        } else {
                            tokoSelect.innerHTML = '<option value="">Tidak ada toko</option>';
                            tokoSelect.disabled = true;
                        }
                    });
            }
        ...
        document.getElementById('filter-toko').addEventListener('change', function() {
            loadProdukData(1);
        });

        // Trigger filter saat username diubah
        document.getElementById('filter-username').addEventListener('change', function() {
            loadProdukData(1);
        });
    ...
    function loadProdukData(page = 1) {
        ...
        const filterUsername = document.getElementById('filter-username') ? document.getElementById('filter-username').value : '';
        const filterToko = document.getElementById('filter-toko') ? document.getElementById('filter-toko').value : '';

        // Update filter indicator
        updateFilterIndicator(filterStatus, filterNama, filterTanggal, filterUsername);
    ...
    function updateFilterIndicator(status, nama, tanggal) {
        ...
        if (nama !== '') {
            activeFilters.push(`Nama / Qr: "${nama}"`);
        }
    ...
    function displayProdukData(products) {
        ...
        tbody.innerHTML = products.map((product, index) => {
            ...
            // Escape single quote untuk JS string
            const safeNama = (product.nama_barang || '').replace(/'/g, "\\'");
            return `
            <tr>
                <td>${rowNumber}</td>
                <td>
                ...
                <td>
                    ...
                    <button class="btn btn-danger btn-sm ms-1 btn-delete-produk" data-id="${product.id}" data-nama="${safeNama}" data-kode="${product.kode_barcode}">
        ...
        tbody.querySelectorAll('.btn-delete-produk').forEach(btn => {
            btn.addEventListener('click', function(e) {
                ...
                if (confirm(`Yakin ingin menghapus produk '${nama}' (Barcode: ${kode})? Stok toko akan otomatis dikurangi jika ada.`)) {
                    fetch('hapus-produk.php', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/x-www-form-urlencoded'
                            },
                            body: 'id=' + encodeURIComponent(id) + '&kode_barcode=' + encodeURIComponent(kode)
                        })
                        .then(r => r.json())
                        .then(res => {
                            if (res.status === 'success') {
                                ...
                                showAlert('error', 'Gagal menghapus produk: ' + (res.message || 'Unknown error'));
                            }
                        })
                        .catch(err => {
                            // Coba ambil response text jika bukan JSON
                            if (err instanceof SyntaxError) {
                                showAlert('error', 'Gagal menghapus produk: Response bukan JSON valid. Cek error PHP di backend.');
                            } else {
                                showAlert('error', 'Gagal menghapus produk: ' + err);
                            }
                        });
                }
            });
    ...
    function getStatusBadge(status) {
        ...
        switch (status) {
            ...
            case 'rusak':
                return '<span class="badge bg-dark">Rusak</span>';
            default:
